...
def collator_graph_data(items, max_node=512, multi_hop_max_dist=20, spatial_pos_max=20, transform_in_collator=False, include_y=False, rich_features=False):
    ...
    for item in items:
        try:
            # Ensure that SMILES is a valid string or item is a valid graph
            if isinstance(item, str):  # If item is SMILES string
                if item == "" or item is None:
                    raise ValueError(f"Invalid SMILES string found: {item}")
                item = smiles2graph(item)  # Convert SMILES to graph
            elif not isinstance(item, Data):  # If item is not a valid Data object
            ...
            # Copy data from item to item_new
            ...
            # Log invalid item and skip it
            logger.warning(f"Skipping item due to invalid data: {e}")
            continue  # Skip the current invalid item

    # Filter out None items from the processed batch
    items_new = [item for item in items_new if item is not None and item['x'].size(0) <= max_node]
    ...
    # Update attention biases where spatial positions exceed the threshold
...
def collator_graph_data(items, max_node=512, multi_hop_max_dist=20, spatial_pos_max=20, transform_in_collator=False, include_y=False, rich_features=False):
    ...
    for item in items:
        try:
            # Ensure that item is not None and that it's a valid SMILES string or graph
            if item is None:
                raise ValueError("Item is None, skipping this item.")
            
            # Check if item is a SMILES string
            if isinstance(item, str):  # If item is SMILES string
                if item == "" or item is None:
                    raise ValueError(f"Invalid SMILES string found: {item}")
            ...
            # If item is not a SMILES string or graph data, raise error
            elif not isinstance(item, Data):  # If item is not a valid Data object
                raise ValueError(f"Invalid data item: {item}")

            # If include_y is True, add labels
            if include_y:
                ys.append(item.y)
            ...
            # Log invalid item and skip it
            logger.warning(f"Skipping item due to invalid data: {e}")
            print(f"Invalid item: {e}")  # Print invalid item for debugging
            continue  # Skip the current invalid item
