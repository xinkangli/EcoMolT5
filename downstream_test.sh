...
then
  for dataset in "${dataset_list[@]}"; do
    CUDA_VISIBLE_DEVICES=$device python  downstream_test.py --zero_shot  --transformer_backbone gimlet --model_name_or_path $ckpt_dir$model_ckpt_name --tokenizer_name t5-small --dataset "$dataset" --runseed 5 --batch_size 40 --grad_accum_step 1   --transform_in_collator --only_test --use_huggingface_pipeline --output_result_to_file $outfile --not_retest_tasks_in_result_file
...
then
  for dataset in "${dataset_list[@]}"; do
    CUDA_VISIBLE_DEVICES=$device python downstream_test.py  --zero_shot  --transformer_backbone kvplm  --model_name_or_path $ckpt_dir$model_ckpt_name/gimlet --dataset "$dataset" --prompt_file prompt_downstream_task.json --runseed 5 --batch_size 40 --grad_accum_step 1  --only_test --output_result_to_file $outfile  --not_retest_tasks_in_result_file
