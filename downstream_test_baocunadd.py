...
def get_num_task(dataset):
    ...
    if dataset == 'tox21':
        return 12
    elif dataset in ['hiv', 'bace', 'bbbp', 'donor', 'esol', 'freesolv', 'lipo','duxing','jiangjie']:
        return 1
    elif dataset == 'pcba':
        return 128
    elif dataset == 'muv':
...
def downstream_task_by_transform(model, train_loader, val_loader, test_loader, prompt=''):
    ...
    if args.zero_shot:
        ...
        record = {
            'text': prompt,  # 文本
            'train_score': train_roc['score'],
            'val_score': val_roc['score'],
            'test_score': test_roc['score']
        }
        records.append(record)  # 将记录添加到列表中
        ...
        if not (test_roc['score'] == 0) and args.output_result_to_file is not None:
            ...
            record = [(args.dataset, model_file, train_roc['score'], val_roc['score'], test_roc['score'], prompt,'train')]
        ...
        # 如果指定了文件路径，则保存到该路径
        if args.output_result_to_file:
            output_file = args.output_result_to_file
        else:
            output_file = "result/result0.csv"  # 如果没有传入路径，则使用默认路径

        os.makedirs(os.path.dirname(output_file), exist_ok=True)  # 确保目录存在
        df.to_csv(output_file, index=False)  # 保存到指定路径
...
if __name__ == '__main__':
    ...
    if not args.use_huggingface_pipeline:
        ...
        if args.split == 'scaffold':
            ...
            train_index_total = torch.arange(len(train_dataset))
            valid_index_total = torch.arange(len(valid_dataset))
            test_index_total = torch.arange(len(test_dataset))
