...
from model.GIMLET.gnn_model import GNN
from transformers.utils import logging
from transformers.modeling_outputs import (
    BaseModelOutputWithPastAndCrossAttentions,
)
from model.graphormer.modules.graphormer_graph_encoder import GraphEncoder
...
@dataclass
class GIMLETOutput(BaseModelOutputWithPastAndCrossAttentions):
...
from .data_utils import get_morgan_fingerprint
...
class GIMLETEncoderStack(T5Stack):
    def __init__(self, config, graph_args,embed_tokens=None):
        ...
        self.fingerprint_projector = nn.Sequential(
            nn.Linear(fingerprint_dim, self.config.d_model),
            nn.ReLU(),
    ...
    def forward(
        ...
        self.graph_repr = hidden_state_graph.transpose(0, 1).mean(dim=1)  # (B, D)
        self.text_repr = hidden_states[:, -1, :]  # (B, D)
        #指纹表征
        if fingerprint is not None:
            self.fingerprint_repr = self.fingerprint_projector(fingerprint)  # (B, D)
        else:
            self.fingerprint_repr = None
        ...
        return GIMLETOutput(
            ...
            fingerprint_repr=self.fingerprint_repr,
        )
...
GraphT5EncoderStack_dict={'gint5':GinT5EncoderStack,
                       'gimlet':GIMLETEncoderStack}
