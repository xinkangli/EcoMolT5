...
from model.GIMLET.contrastive import contrastive_loss
...
class GraphT5TransformerForConditionalGeneration(T5ForConditionalGeneration):
    ...
    def forward(
        ...
        if labels is not None:
            ...
            ### ---- Contrastive Loss Section ---- ###
            if self.training and hasattr(self.config, 'contrastive_alpha') and self.config.contrastive_alpha > 0:
                text_repr = encoder_outputs.last_hidden_state.mean(dim=1)  # (batch, hidden)
                graph_repr = getattr(self.encoder, 'graph_repr', None)  # (batch, hidden)

                if graph_repr is not None:
                    cl_loss = contrastive_loss(text_repr, graph_repr, temperature=0.07)
                    loss = loss + self.config.contrastive_alpha * cl_loss
            ### ----------------------------------- ###
