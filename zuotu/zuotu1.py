...
# ---------------------------
def parse_args():
    ap = argparse.ArgumentParser(description="BACE t-SNE (local CSV): four-view visualization with Times New Roman")
...
def save_plot(Z, labels, title, filename):
    ...
    import matplotlib
    matplotlib.rcParams["font.family"] = "Times New Roman"  # 设置字体为新罗马
    plt.figure(figsize=(6, 6))
    plt.scatter(Z[:, 0], Z[:, 1], s=10, c=labels, cmap="coolwarm")
...
def main():
    ...
    save_plot(Z_graph, labels, "BACE • Graph-only (Morgan)", os.path.join(args.save_dir, "graph_only.png"))
    ...
    save_plot(Z_text, labels, "BACE • Text-only (DistilBERT)", os.path.join(args.save_dir, "text_only.png"))
    ...
    save_plot(Z_fusion, labels, "BACE • Graph+Text (Late fusion)", os.path.join(args.save_dir, "fusion.png"))

    # Supervised UMAP
    print("==> Supervised UMAP (clearly separated)...")
    Z_umap = run_umap_supervised(X_fusion, labels, random_state=args.seed)
    save_plot(Z_umap, labels, "BACE • Graph+Text (Supervised UMAP, Fully Separated)", os.path.join(args.save_dir, "umap_supervised.png"))

    print(f"[DONE] All figures saved in folder: {args.save_dir}")
